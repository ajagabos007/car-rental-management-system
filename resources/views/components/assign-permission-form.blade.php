<div>
 ...
 -->
    <form action="{{route('admin.users.assign_permission', $user)}}" enctype="multipart/form-data" method="post">
        @csrf
        <div class="grid md:grid">
            ...
            <div class="flex items-center space-x-3 font-semibold text-gray-900 text-xl leading-8">
                ...
                <x-jet-label class="text-lg" for="permission" value="{{ $user->name.' s ' . __('direct permissions') }}" />
            </div>
            <ul class="w-full grid grid-cols-2 lg:grid-cols-4  rounded {{ $errors->has('permission') ? 'border-pink-500 text-pink-600 focus:border-pink-500 focus:ring-pink-500' : '' }}" >
                @foreach($permissions as $permission)
                    @if(old('permission_'.$permission->id) )
                        ...
                        @endif
                    @else 
                            @if($user->permissions()->where('permission_id', $permission->id)->get()->first())
                                <li> 
                                    <x-jet-checkbox  :value="$permission->id" name="permission_{{$permission->id}}" checked/>
                                    <label for="" class="text-sm text-gray-600"> {{$permission->name}}</label>
                                </li>
