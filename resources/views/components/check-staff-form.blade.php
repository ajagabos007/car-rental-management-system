<div>
 ...
 -->
  <form action="{{route('admin.users.staffs.update', $user)}}"  enctype="multipart/form-data" method="post">
    @csrf 
    @method('put')

    @if($user->is_staff)
      <x-jet-checkbox  :value="$user->id" name="staff" checked/>
    @else 
      <x-jet-checkbox  :value="$user->id" name="staff"/>
    @endif
    <label for="" class="text-sm text-gray-600"> staff </label>
    <span class="flex items-center justify-end"  x-data="{ open: false }">
